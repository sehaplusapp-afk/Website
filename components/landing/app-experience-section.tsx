"use client";

import {
  motion,
  useScroll,
  useTransform,
  useMotionValueEvent,
  AnimatePresence,
} from "framer-motion";
import { useLanguage } from "@/lib/language-context";
import { useRef, useState } from "react";
import {
  Video,
  Users,
  Stethoscope,
  Calendar,
  Bell,
  ChevronRight,
} from "lucide-react";
import Image from "next/image";

const features = [
  {
    icon: Video,
    titleKey: "experience.video.title",
    descKey: "experience.video.desc",
    color: "#1B8AA8",
    gradient: "from-[#1B8AA8] to-[#2FA6C4]",
    screenContent: (
      <div className="w-full h-full pt-7">
        <div className="relative w-full h-full">
          <Image
            src="../../images/call.png"
            alt="Video consultation"
            fill
            className="object-cover"
            priority={false}
          />
        </div>
      </div>
    ),
  },
  {
    icon: Users,
    titleKey: "experience.doctors.title",
    descKey: "experience.doctors.desc",
    color: "#2FA6C4",
    gradient: "from-[#2FA6C4] to-[#6FD1E6]",
    screenContent: (
      <div className="w-full h-full pt-7">
        <div className="relative w-full h-full">
          <Image
            src="../../images/seha-dr-screen.png"
            alt="Browse doctors"
            fill
            className="object-contain bg-[#F5FBFD]"
            priority={false}
          />
        </div>
      </div>
    ),
  },
  {
    icon: Stethoscope,
    titleKey: "experience.specialties.title",
    descKey: "experience.specialties.desc",
    color: "#0E5F73",
    gradient: "from-[#0E5F73] to-[#1B8AA8]",
    screenContent: (
      <div className="w-full h-full pt-7">
        <div className="relative w-full h-full">
          <Image
            src="../../images/medical-specialties.png"
            alt="Medical specialties"
            fill
            className="object-cover"
            priority={false}
          />
        </div>
      </div>
    ),
  },
  {
    icon: Calendar,
    titleKey: "experience.booking.title",
    descKey: "experience.booking.desc",
    color: "#6FD1E6",
    gradient: "from-[#6FD1E6] to-[#2FA6C4]",
    screenContent: (
      <div className="w-full h-full pt-7">
        <div className="relative w-full h-full">
          <Image
            src="../../images/appoinment.png"
            alt="Easy booking"
            fill
            className="object-cover"
            priority={false}
          />
        </div>
      </div>
    ),
  },
  {
    icon: Bell,
    titleKey: "experience.notifications.title",
    descKey: "experience.notifications.desc",
    color: "#1B8AA8",
    gradient: "from-[#1B8AA8] to-[#0E5F73]",
    screenContent: (
      <div className="w-full h-full pt-7">
        <div className="relative w-full h-full">
          <Image
            src="../../images/notification.png"
            alt="Smart notifications"
            fill
            className="object-cover"
            priority={false}
          />
        </div>
      </div>
    ),
  },
];

const mobileFloatingPositions = [
  {
    title: "top-[18%] left-4",
    desc: "bottom-[27%] -left-4",
  },
  {
    title: "top-[3%] right-4",
    desc: "bottom-[5%] left-1",
  },
  {
    title: "top-[4%] left-4",
    desc: "bottom-[15%] right-4",
  },
  {
    title: "top-[18%] right-4",
    desc: "bottom-[15%] left-4",
  },
  {
    title: "top-[22%] left-4",
    desc: "bottom-[14%] right-4",
  },
];

export function AppExperienceSection() {
  const { t, dir } = useLanguage();
  const containerRef = useRef<HTMLDivElement>(null);
  const lastIndexRef = useRef(0);
  const [activeIndex, setActiveIndex] = useState(0);

  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start start", "end end"],
  });

  const progressHeight = useTransform(scrollYProgress, [0, 1], ["0%", "100%"]);

  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    const nextIndex = Math.min(
      Math.round(latest * features.length),
      features.length - 1,
    );

    if (lastIndexRef.current !== nextIndex) {
      lastIndexRef.current = nextIndex;
      setActiveIndex(nextIndex);
    }
  });

  const activeFeature = features[activeIndex];
  const ActiveMobileIcon = activeFeature.icon;

  return (
<section
  id="app-experience"
  ref={containerRef}
  className="relative min-h-[300vh] lg:min-h-[260vh] xl:min-h-[280vh]"
  style={{ direction: dir }}
>
      {/* Mobile Header - outside sticky */}
      <div className="lg:hidden relative z-10 px-4 sm:px-6 pt-6 pb-4 text-center">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#1B8AA8]/10 text-[#1B8AA8] text-sm font-semibold mb-4 border border-[#1B8AA8]/10">
          <Stethoscope className="w-4 h-4" />
          {dir === "rtl" ? "تجربة سلسة" : "Seamless Experience"}
        </div>

        <h2 className="text-xl sm:text-3xl font-bold text-[#0E5F73] mb-2 leading-tight">
          {t("experience.title")}
        </h2>

        <p className="text-sm sm:text-base text-[#0E5F73]/60 max-w-2xl mx-auto">
          {t("experience.subtitle")}
        </p>

        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: [0, 1, 0] }}
          transition={{ duration: 3, repeat: Infinity }}
          className="flex justify-center mt-4"
        >
          <div className="w-6 h-10 border border-[#1B8AA8]/30 rounded-full flex justify-center">
            <motion.div
              animate={{ y: [0, 12, 0] }}
              transition={{ duration: 1.5, repeat: Infinity }}
              className="w-1.5 h-1.5 bg-[#1B8AA8] rounded-full mt-2"
            />
          </div>
        </motion.div>
      </div>

      <div className="sticky top-[76px] h-[calc(100svh-76px)] lg:top-0 lg:h-[100svh] overflow-hidden">
        {/* Background */}
        <div className="absolute inset-0 bg-gradient-to-b from-white via-[#F5FBFD] to-white" />

        {/* Softer orb */}
        <motion.div
          animate={{ opacity: [0.18, 0.26, 0.18] }}
          transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
          className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[300px] h-[300px] bg-gradient-radial from-[#1B8AA8]/6 to-transparent rounded-full blur-xl"
        />

        {/* Mobile side progress */}
        <div className="lg:hidden absolute right-3 top-1/2 -translate-y-1/2 h-36 w-1 bg-[#1B8AA8]/10 rounded-full overflow-hidden z-30">
          <motion.div
            style={{ height: progressHeight }}
            className="w-full bg-gradient-to-b from-[#1B8AA8] to-[#2FA6C4] rounded-full"
          />
        </div>

        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full h-full lg:flex lg:flex-col lg:justify-center lg:py-6 xl:py-8">
          {/* Header */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
            className="hidden lg:block text-center mb-8 xl:mb-10 shrink-0"
          >
            <motion.div
              initial={{ opacity: 0, scale: 0.9 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#1B8AA8]/10 text-[#1B8AA8] text-sm font-semibold mb-4 md:mb-5 border border-[#1B8AA8]/10"
            >
              <Stethoscope className="w-4 h-4" />
              {dir === "rtl" ? "تجربة سلسة" : "Seamless Experience"}
            </motion.div>

            <h2 className="text-xl sm:text-3xl md:text-5xl font-bold text-[#0E5F73] mb-2 md:mb-4 text-balance leading-tight">
              {t("experience.title")}
            </h2>

            <p className="text-sm sm:text-base md:text-lg text-[#0E5F73]/60 max-w-2xl mx-auto">
              {t("experience.subtitle")}
            </p>
          </motion.div>

          {/* Desktop */}
          <div className="hidden lg:grid lg:grid-cols-2 gap-8 xl:gap-12 items-center flex-1 min-h-0">
            {/* Phone Mockup */}
            <div className="relative flex justify-center order-2 lg:order-1">
              <div className="relative">
                <div
                  className="absolute -inset-10 rounded-full blur-2xl -z-10"
                  style={{
                    background: `radial-gradient(circle, ${activeFeature.color}18 0%, transparent 70%)`,
                  }}
                />

                <motion.div
                  initial={{ opacity: 0, y: 24 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.7 }}
                  className="w-[clamp(220px,20vw,300px)] aspect-[300/610] bg-gradient-to-br from-[#1B8AA8] via-[#1B8AA8] to-[#0E5F73] rounded-[2.8rem] xl:rounded-[3rem] p-2.5 xl:p-3 shadow-xl shadow-[#1B8AA8]/25"
                >
                  <div className="w-full h-full bg-[#F5FBFD] rounded-[2.3rem] md:rounded-[2.5rem] overflow-hidden relative">
                    <div className="absolute top-0 left-0 right-0 h-7 md:h-8 bg-gradient-to-b from-[#1B8AA8] to-[#2FA6C4] flex items-center justify-center z-10">
                      <div className="w-16 md:w-20 h-4 md:h-5 bg-black/20 rounded-full" />
                    </div>

                    <div className="relative h-full overflow-hidden">
                      <AnimatePresence mode="popLayout">
                        <motion.div
                          key={activeIndex}
                          initial={{ opacity: 0, y: 6 }}
                          exit={{ opacity: 0, y: -4 }}
                          animate={{ opacity: 1, y: 0, scale: 1 }}
                          transition={{
                            duration: 0.3,
                            ease: [0.22, 1, 0.36, 1],
                          }}
                          className="absolute inset-0"
                        >
                          {activeFeature.screenContent}
                        </motion.div>
                      </AnimatePresence>
                    </div>
                  </div>
                </motion.div>

                <div className="absolute inset-0 -z-20 flex items-center justify-center pointer-events-none">
                  <div className="relative w-[420px] h-[420px]">
                    <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle,_rgba(27,138,168,0.12)_0%,_rgba(47,166,196,0.07)_38%,_transparent_72%)] blur-2xl" />
                    <div className="absolute inset-[14%] rounded-full bg-[radial-gradient(circle,_rgba(111,209,230,0.08)_0%,_rgba(27,138,168,0.05)_42%,_transparent_72%)] blur-xl" />
                  </div>
                </div>

                <svg
                  className="absolute -inset-20 -z-10 hidden md:block pointer-events-none"
                  viewBox="0 0 560 800"
                  fill="none"
                >
                  <path
                    d="M442 118C495 168 514 250 500 334C486 418 430 482 384 540C344 590 328 652 352 714"
                    stroke="url(#orbitGradientMain)"
                    strokeWidth="2.2"
                    strokeLinecap="round"
                    strokeDasharray="12 14"
                    opacity="0.5"
                  />
                  <path
                    d="M118 666C72 616 54 544 66 466C80 380 128 324 162 280C194 238 208 184 184 132"
                    stroke="url(#orbitGradientSoft)"
                    strokeWidth="1.9"
                    strokeLinecap="round"
                    strokeDasharray="9 16"
                    opacity="0.32"
                  />
                  <path
                    d="M408 170C438 214 446 270 438 326C430 384 400 430 372 472"
                    stroke="url(#orbitGradientMini)"
                    strokeWidth="1.4"
                    strokeLinecap="round"
                    strokeDasharray="4 12"
                    opacity="0.22"
                  />

                  <defs>
                    <linearGradient
                      id="orbitGradientMain"
                      x1="352"
                      y1="118"
                      x2="500"
                      y2="714"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stopColor="#6FD1E6" />
                      <stop offset="1" stopColor="#1B8AA8" />
                    </linearGradient>

                    <linearGradient
                      id="orbitGradientSoft"
                      x1="66"
                      y1="132"
                      x2="184"
                      y2="666"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stopColor="#E7F8FB" />
                      <stop offset="1" stopColor="#2FA6C4" />
                    </linearGradient>

                    <linearGradient
                      id="orbitGradientMini"
                      x1="372"
                      y1="170"
                      x2="446"
                      y2="472"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stopColor="#BCECF4" />
                      <stop offset="1" stopColor="#1B8AA8" />
                    </linearGradient>
                  </defs>
                </svg>

                <motion.div
                  animate={{ y: [0, -6, 0], rotate: [0, 2, 0] }}
                  transition={{
                    duration: 6,
                    repeat: Infinity,
                    ease: "easeInOut",
                  }}
                  className="absolute top-8 -right-12 z-30 hidden md:flex h-15 w-15 items-center justify-center rounded-[22px] border border-white/70 bg-white/60 backdrop-blur-md shadow-[0_14px_36px_rgba(27,138,168,0.10)]"
                >
                  <div className="absolute inset-[1px] rounded-[21px] bg-[linear-gradient(180deg,rgba(255,255,255,0.42),rgba(255,255,255,0.08))]" />
                  <div className="absolute top-1.5 left-1.5 h-2.5 w-2.5 rounded-full bg-white/45 blur-[1px]" />
                  <Video className="relative z-10 w-6 h-6 text-[#1B8AA8]" />
                </motion.div>

                <motion.div
                  animate={{ y: [0, 7, 0], rotate: [0, -2, 0] }}
                  transition={{
                    duration: 6.8,
                    repeat: Infinity,
                    ease: "easeInOut",
                    delay: 0.35,
                  }}
                  className="absolute bottom-8 -left-12 z-30 hidden md:flex h-15 w-15 items-center justify-center rounded-[22px] border border-white/70 bg-white/60 backdrop-blur-md shadow-[0_14px_36px_rgba(27,138,168,0.10)]"
                >
                  <div className="absolute inset-[1px] rounded-[21px] bg-[linear-gradient(180deg,rgba(255,255,255,0.42),rgba(255,255,255,0.08))]" />
                  <div className="absolute top-1.5 left-1.5 h-2.5 w-2.5 rounded-full bg-white/45 blur-[1px]" />
                  <Stethoscope className="relative z-10 w-6 h-6 text-[#1B8AA8]" />
                </motion.div>

                <div className="absolute top-24 -left-7 hidden md:flex flex-col gap-2.5 z-20 pointer-events-none">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#6FD1E6]/70 shadow-[0_0_8px_rgba(111,209,230,0.28)]" />
                  <span className="w-2 h-2 rounded-full bg-[#2FA6C4]/55" />
                  <span className="w-1.5 h-1.5 rounded-full bg-[#1B8AA8]/45" />
                </div>

                <div className="absolute bottom-24 -right-7 hidden md:flex flex-col gap-2.5 z-20 pointer-events-none">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#1B8AA8]/45" />
                  <span className="w-2 h-2 rounded-full bg-[#2FA6C4]/55" />
                  <span className="w-1.5 h-1.5 rounded-full bg-[#6FD1E6]/70 shadow-[0_0_8px_rgba(111,209,230,0.28)]" />
                </div>
              </div>
            </div>

            {/* Feature List */}
            <div className="space-y-3 order-1 lg:order-2 min-h-0">
              {features.map((feature, index) => {
                const Icon = feature.icon;
                const isActive = activeIndex === index;

                return (
                  <motion.div
                    key={index}
                    initial={{ opacity: 0, x: dir === "rtl" ? -20 : 20 }}
                    whileInView={{ opacity: 1, x: 0 }}
                    viewport={{ once: true }}
                    transition={{
                      delay: index * 0.08,
                      duration: 0.55,
                      ease: [0.22, 1, 0.36, 1],
                    }}
                    className={`relative p-4 xl:p-5 rounded-2xl transition-all duration-500 cursor-pointer border ${
                      isActive
                        ? "bg-white shadow-lg shadow-[#1B8AA8]/10 border-[#1B8AA8]/20"
                        : "bg-white/60 hover:bg-white/85 border-transparent hover:border-[#1B8AA8]/10"
                    }`}
                    onClick={() => setActiveIndex(index)}
                  >
                    <motion.div
                      initial={{ scaleY: 0 }}
                      animate={{ scaleY: isActive ? 1 : 0 }}
                      transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
                      className={`absolute ${
                        dir === "rtl" ? "right-0" : "left-0"
                      } top-0 bottom-0 w-1 bg-gradient-to-b ${feature.gradient} rounded-full origin-top`}
                    />

                    <div className="flex items-start gap-4">
                      <motion.div
                        animate={{ scale: isActive ? 1 : 0.97 }}
                        transition={{ duration: 0.25 }}
                        className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center flex-shrink-0 transition-all duration-300 ${
                          isActive
                            ? `bg-gradient-to-br ${feature.gradient} shadow-lg`
                            : "bg-[#F5FBFD]"
                        }`}
                        style={{
                          boxShadow: isActive
                            ? `0 8px 24px -8px ${feature.color}45`
                            : "none",
                        }}
                      >
                        <Icon
                          className={`w-5 h-5 md:w-6 md:h-6 transition-colors duration-300 ${
                            isActive ? "text-white" : "text-[#1B8AA8]"
                          }`}
                        />
                      </motion.div>

                      <div className="flex-1 min-w-0">
                        <motion.h3
                          animate={{
                            y: isActive ? [6, 0] : 0,
                            opacity: isActive ? [0.7, 1] : 1,
                          }}
                          transition={{ duration: 0.25 }}
                          className={`text-base md:text-lg font-semibold mb-1 transition-colors duration-300 ${
                            isActive ? "text-[#1B8AA8]" : "text-[#0E5F73]"
                          }`}
                        >
                          {t(feature.titleKey)}
                        </motion.h3>

                        <motion.p
                          animate={{
                            y: isActive ? [8, 0] : 0,
                            opacity: isActive ? [0.65, 1] : 1,
                          }}
                          transition={{
                            duration: 0.28,
                            delay: isActive ? 0.04 : 0,
                          }}
                          className="text-[#0E5F73]/60 text-sm leading-relaxed line-clamp-2"
                        >
                          {t(feature.descKey)}
                        </motion.p>
                      </div>

                      <motion.div
                        animate={{
                          x: isActive ? 0 : -4,
                          opacity: isActive ? 1 : 0,
                        }}
                        transition={{ duration: 0.25 }}
                        className="flex-shrink-0 hidden sm:block"
                      >
                        <ChevronRight
                          className={`w-5 h-5 text-[#1B8AA8] ${
                            dir === "rtl" ? "rotate-180" : ""
                          }`}
                        />
                      </motion.div>
                    </div>
                  </motion.div>
                );
              })}
            </div>
          </div>

          {/* Mobile */}
          <div className="lg:hidden h-full flex flex-col items-center justify-start pt-2 pb-4">
            <div className="relative w-full flex-1 min-h-0 flex items-start justify-center">
              <div className="relative w-full max-w-[380px] mx-auto h-full flex flex-col items-center justify-center px-3">
                <div className="relative w-full flex flex-col items-center justify-center">
                  {/* Floating Title */}
                  {/* Floating Title */}
                  <AnimatePresence mode="popLayout">
                    <motion.div
                      key={`mobile-title-${activeIndex}`}
                      initial={{ opacity: 0, y: 6, scale: 0.99 }}
                      animate={{ opacity: 1, y: 0, scale: 1 }}
                      exit={{ opacity: 0, y: -4, scale: 0.99 }}
                      transition={{ duration: 0.14, ease: [0.22, 1, 0.36, 1] }}
                      className={`absolute z-40 ${mobileFloatingPositions[activeIndex].title}`}
                    >
                      <div className="flex items-center gap-2.5 rounded-full border border-white/75 bg-white/92 backdrop-blur-md shadow-[0_10px_24px_rgba(27,138,168,0.12)] px-3 py-2.5 min-w-[170px] max-w-[220px]">
                        <div
                          className={`w-9 h-9 rounded-full bg-gradient-to-br ${activeFeature.gradient} flex items-center justify-center shrink-0`}
                        >
                          <ActiveMobileIcon className="w-4.5 h-4.5 text-white" />
                        </div>

                        <span className="flex-1 min-w-0 text-[13.5px] font-semibold text-[#0E5F73] leading-tight">
                          {t(activeFeature.titleKey)}
                        </span>
                      </div>
                    </motion.div>
                  </AnimatePresence>

                  {/* Floating Description */}
                  {/* Floating Description */}
                  <AnimatePresence mode="popLayout">
                    <motion.div
                      key={`mobile-desc-${activeIndex}`}
                      initial={{ opacity: 0, y: 7, scale: 0.99 }}
                      animate={{ opacity: 1, y: 0, scale: 1 }}
                      exit={{ opacity: 0, y: -5, scale: 0.99 }}
                      transition={{ duration: 0.15, ease: [0.22, 1, 0.36, 1] }}
                      className={`absolute z-40 ${mobileFloatingPositions[activeIndex].desc}`}
                    >
                      <div className="rounded-[22px] border border-white/75 bg-white/90 backdrop-blur-md shadow-[0_12px_28px_rgba(27,138,168,0.10)] px-4 py-3 w-[220px]">
                        <p className="text-[12.5px] leading-[1.45] text-[#0E5F73]/75 line-clamp-2">
                          {t(activeFeature.descKey)}
                        </p>
                      </div>
                    </motion.div>
                  </AnimatePresence>

                  {/* Glow */}
                  <div
                    className="absolute inset-0 rounded-full blur-2xl -z-10"
                    style={{
                      background: `radial-gradient(circle, ${activeFeature.color}18 0%, transparent 70%)`,
                    }}
                  />

                  {/* Phone + dots */}
                  <div className="relative z-20 flex flex-col items-center">
                    <motion.div
                      initial={{ opacity: 0.92 }}
                      animate={{ opacity: 1 }}
                      transition={{ duration: 0.2 }}
                      className="relative w-[clamp(235px,72vw,292px)] max-w-full aspect-[300/610] bg-gradient-to-br from-[#1B8AA8] via-[#1B8AA8] to-[#0E5F73] rounded-[2.7rem] p-2.5 shadow-xl shadow-[#1B8AA8]/25 mx-auto"
                    >
                      <div className="w-full h-full bg-[#F5FBFD] rounded-[2.2rem] overflow-hidden relative">
                        <div className="absolute top-0 left-0 right-0 h-7 bg-gradient-to-b from-[#1B8AA8] to-[#2FA6C4] flex items-center justify-center z-10">
                          <div className="w-16 h-4 bg-black/20 rounded-full" />
                        </div>

                        <AnimatePresence mode="popLayout">
                          <motion.div
                            key={activeIndex}
                            initial={{ opacity: 0, y: 8, scale: 0.995 }}
                            animate={{ opacity: 1, y: 0, scale: 1 }}
                            exit={{ opacity: 0, y: -6, scale: 0.995 }}
                            transition={{
                              duration: 0.35,
                              ease: [0.25, 1, 0.5, 1], // smoother easing
                            }}
                            className="absolute inset-0"
                          >
                            {activeFeature.screenContent}
                          </motion.div>
                        </AnimatePresence>
                      </div>
                    </motion.div>

                    {/* Dots */}
                    <div className="mt-4 z-30 flex justify-center gap-2">
                      {features.map((feature, index) => (
                        <button
                          key={index}
                          onClick={() => setActiveIndex(index)}
                          className={`h-2.5 rounded-full transition-all duration-300 ${
                            activeIndex === index
                              ? `w-10 bg-gradient-to-r ${feature.gradient}`
                              : "w-2.5 bg-[#1B8AA8]/25"
                          }`}
                        />
                      ))}
                    </div>
                  </div>

                  {/* Mobile layered ambient glow */}
                  <div className="absolute inset-0 -z-20 flex items-center justify-center pointer-events-none">
                    <div className="relative w-[260px] h-[260px] sm:w-[300px] sm:h-[300px]">
                      <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle,_rgba(27,138,168,0.12)_0%,_rgba(47,166,196,0.07)_38%,_transparent_72%)] blur-2xl" />
                      <div className="absolute inset-[14%] rounded-full bg-[radial-gradient(circle,_rgba(111,209,230,0.10)_0%,_rgba(27,138,168,0.05)_42%,_transparent_72%)] blur-xl" />
                    </div>
                  </div>

                  {/* Mobile orbit lines */}
                  <svg
                    className="absolute -inset-6 -z-10 pointer-events-none"
                    viewBox="0 0 420 620"
                    fill="none"
                  >
                    <path
                      d="M332 96C374 136 388 202 378 268C366 334 324 384 288 432C258 472 246 520 264 570"
                      stroke="url(#mobileOrbitGradientMain)"
                      strokeWidth="2.4"
                      strokeLinecap="round"
                      strokeDasharray="6 10"
                      opacity="0.7"
                    />
                    <path
                      d="M92 520C56 480 42 422 50 360C60 294 98 250 126 214C152 182 164 140 146 98"
                      stroke="url(#mobileOrbitGradientSoft)"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeDasharray="5 12"
                      opacity="0.45"
                    />

                    <defs>
                      <linearGradient
                        id="mobileOrbitGradientMain"
                        x1="264"
                        y1="96"
                        x2="378"
                        y2="570"
                        gradientUnits="userSpaceOnUse"
                      >
                        <stop stopColor="#6FD1E6" />
                        <stop offset="1" stopColor="#1B8AA8" />
                      </linearGradient>

                      <linearGradient
                        id="mobileOrbitGradientSoft"
                        x1="50"
                        y1="98"
                        x2="146"
                        y2="520"
                        gradientUnits="userSpaceOnUse"
                      >
                        <stop stopColor="#E7F8FB" />
                        <stop offset="1" stopColor="#2FA6C4" />
                      </linearGradient>
                    </defs>
                  </svg>

                  {/* Mobile top-right premium icon */}
                  <div className="absolute top-[16%] right-0 z-30 flex h-11 w-11 items-center justify-center rounded-[16px] border border-white/70 bg-white/80 shadow-[0_10px_24px_rgba(27,138,168,0.10)] sm:h-12 sm:w-12">
                    <div className="absolute inset-[1px] rounded-[15px] bg-[linear-gradient(180deg,rgba(255,255,255,0.42),rgba(255,255,255,0.08))]" />
                    <div className="absolute top-1 left-1 h-2 w-2 rounded-full bg-white/45 blur-[1px]" />
                    <Video className="relative z-10 w-4 h-4 sm:w-5 sm:h-5 text-[#1B8AA8]" />
                  </div>

                  {/* Mobile bottom-left premium icon */}
                  <div className="absolute bottom-[18%] left-0 z-30 flex h-11 w-11 items-center justify-center rounded-[16px] border border-white/70 bg-white/80 shadow-[0_10px_24px_rgba(27,138,168,0.10)] sm:h-12 sm:w-12">
                    <div className="absolute inset-[1px] rounded-[15px] bg-[linear-gradient(180deg,rgba(255,255,255,0.42),rgba(255,255,255,0.08))]" />
                    <div className="absolute top-1 left-1 h-2 w-2 rounded-full bg-white/45 blur-[1px]" />
                    <Stethoscope className="relative z-10 w-4 h-4 sm:w-5 sm:h-5 text-[#1B8AA8]" />
                  </div>

                  {/* Mobile ambient dots */}
                  <div className="absolute top-[18%] left-1 z-20 flex flex-col gap-2 pointer-events-none">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#6FD1E6]/70 shadow-[0_0_8px_rgba(111,209,230,0.35)]" />
                    <span className="w-1.5 h-1.5 rounded-full bg-[#2FA6C4]/55" />
                  </div>

                  <div className="absolute bottom-[18%] right-1 z-20 flex flex-col gap-2 pointer-events-none">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#1B8AA8]/45" />
                    <span className="w-1.5 h-1.5 rounded-full bg-[#6FD1E6]/70 shadow-[0_0_8px_rgba(111,209,230,0.35)]" />
                  </div>
                </div>
              </div>
            </div>
          </div>
          {/* End Mobile */}
        </div>
      </div>
    </section>
  );
}
